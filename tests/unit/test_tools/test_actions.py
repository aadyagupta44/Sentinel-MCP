...
class TestDisableUser:
    async def test_first_call_returns_proposal(self):
        ...
        result = await _execute_disable_user({
            "email": "alice.hr@acmecorp.com", "reason": "compromise", "confirmed": False, "confirmation_token": "",
        })
    ...
    async def test_invalid_email_returns_error(self):
        from sentinel.tools.actions import _execute_disable_user
        result = await _execute_disable_user({"email": "notanemail", "reason": "test", "confirmed": False, "confirmation_token": ""})
        assert result["code"] == "INVALID_PARAMETER"

    async def test_missing_reason_returns_error(self):
        from sentinel.tools.actions import _execute_disable_user
        result = await _execute_disable_user({"email": "alice.hr@acmecorp.com", "reason": "", "confirmed": False, "confirmation_token": ""})
...
class TestBlockIP:
    async def test_first_call_returns_proposal(self):
        ...
        result = await _execute_block_ip({
            "ip_address": "185.220.101.34", "reason": "c2", "confirmed": False, "confirmation_token": "",
        })
        assert result["action_type"] == "block_ip"
        assert "confirmation_token" in result
    ...
    async def test_missing_ip_returns_error(self):
        from sentinel.tools.actions import _execute_block_ip
        result = await _execute_block_ip({"ip_address": "", "reason": "test", "confirmed": False, "confirmation_token": ""})
        assert result["code"] == "MISSING_PARAMETER"
...
class TestKillProcess:
    async def test_first_call_returns_proposal(self):
        ...
        result = await _execute_kill_process({
            "hostname": "LAPTOP-001", "pid": 4821, "reason": "malware", "confirmed": False, "confirmation_token": "",
        })
    ...
    async def test_invalid_pid_returns_error(self):
        from sentinel.tools.actions import _execute_kill_process
        result = await _execute_kill_process({"hostname": "LAPTOP-001", "pid": -1, "reason": "test", "confirmed": False, "confirmation_token": ""})
        assert result["code"] == "INVALID_PARAMETER"

    async def test_missing_hostname_returns_error(self):
        from sentinel.tools.actions import _execute_kill_process
        result = await _execute_kill_process({"hostname": "", "pid": 1234, "reason": "test", "confirmed": False, "confirmation_token": ""})
        assert result["code"] == "MISSING_PARAMETER"
