...
class TestDeviceProcesses:
    async def test_returns_process_list(self):
        ...
        result = await _execute_device_processes({"hostname": "LAPTOP-HR-03", "time_window_minutes": 60})
    ...
    async def test_suspicious_count_matches(self):
        ...
        result = await _execute_device_processes({"hostname": "LAPTOP-HR-03", "time_window_minutes": 30})
    ...
    async def test_window_is_capped(self):
        ...
        result = await _execute_device_processes({"hostname": "HOST-001", "time_window_minutes": 99999})
...
class TestNetworkConnections:
    async def test_returns_connection_list(self):
        ...
        result = await _execute_network_connections({"hostname": "LAPTOP-HR-03", "time_window_minutes": 60})
        assert result["hostname"] == "LAPTOP-HR-03"
